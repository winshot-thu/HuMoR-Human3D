...
SMPL_MODEL_PATH = '/apdcephfs/share_1290939/shaolihuang/data/VIBE-pare/pare_data/body_models/smpl/SMPL_NEUTRAL.pkl'
smpl = pkl.load(open(SMPL_MODEL_PATH, 'rb'), encoding='latin1')
J_regressor = smpl['J_regressor']
J_regressor = J_regressor.toarray()
J_regressor = torch.from_numpy(J_regressor).to(torch.float32).to("cuda")
...
class FittingLoss(nn.Module):
    ...
    def root_fit(self, observed_data, pred_data):
        ...
        if 'verts3d' in observed_data and 'verts3d' in pred_data and self.loss_weights['verts3d'] > 0.0:
            if self.datatype == 'PARE':
                cur_loss = self.verts3d_loss(observed_data['verts3d'], pred_data['verts3d'], observed_data['verts3d_orig'], pred_data['verts3d_orig'])
            else:
    ...
    def joints2d_loss(self, joints2d_obs, joints3d_pred, joints3d_extra_pred, cam_t=None, cam_R=None, debug_img=None):
        ...
        if self.datatype == 'PARE':
            joints2d_obs_conf = torch.ones_like(joints2d_obs[:,:,:,0:1]).float()
        else:
    ...
    def verts3d_loss(self, verts3d_obs, verts3d_pred, verts3d_orig_obs=None, verts3d_orig_pred=None):
        if verts3d_orig_obs is not None and verts3d_orig_pred is not None:
            B, T, V, _ = verts3d_orig_obs.shape
            joints3d_obs = vertices2joints(J_regressor=J_regressor, vertices=verts3d_orig_obs.reshape(B*T, V, 3)).reshape(B, T, 24, 3)
            joints3d_pred = vertices2joints(J_regressor=J_regressor, vertices=verts3d_orig_pred.reshape(B*T, V, 3)).reshape(B, T, 24, 3)

            gt_pelvis = joints3d_obs[:,:, 0,:] 
            gt_keypoints_3d = verts3d_obs - gt_pelvis[:,:, None, :]
            pred_pelvis = joints3d_pred[:,:,0,:] 
            pred_keypoints_3d = verts3d_pred - pred_pelvis[:,:, None, :]

            verts3d_obs, verts3d_pred = gt_keypoints_3d, pred_keypoints_3d
