...
from datasets.pare_dataset import PAREDataset
import joblib
...
def main(args, config_file):
    ...
    if args.data_type == 'AMASS':
    ...
    elif args.data_type == 'PARE':
        video_preprocess_path = os.path.join(args.out, 'rgb_preprocess')
        pare_results_path = video_preprocess_path + '/results.pt'
        img_folder = os.path.join(video_preprocess_path, 'raw_frames')
        ...
        if not os.path.exists(video_preprocess_path):
            ...
            tracking_results = tester.run_tracking(img_folder)

            pare_results = tester.run_on_video(tracking_results, img_folder, orig_width, orig_height)
            joblib.dump(pare_results[1], pare_results_path)
            
            print('cleaning gpu memory cache...')
            del tester
            del pare_results
        ...
        if args.rgb_intrinsics is None: # true
            cam_mat = np.array([[DEFAULT_FOCAL_LEN[0], 0.0, img_shape[1] / 2.],
                                [0.0, DEFAULT_FOCAL_LEN[1], img_shape[0] / 2.],
                                [0.0, 0.0, 1.0]])
        vid_name = '.'.join(args.data_path.split('/')[-1].split('.')[:-1])

        dataset = PAREDataset(pare_results_path, img_folder, cat_mat=cam_mat, seq_len=args.rgb_seq_len, overlap_len=args.rgb_overlap_len, vid_name=vid_name)
    ...
    if (args.data_type == 'RGB' or args.data_type == 'PARE') and args.save_results:
        all_res_out_paths = []
    ...
    for i, data in enumerate(data_loader):
        ...
        if cur_res_out_paths is not None and (args.data_type == 'RGB' or args.data_type=='PARE') and args.save_results:
